 ...
 * @param {boolean} [options.includePrivateTlds]
 * @returns {object|null}
 */
function parseDomain(url, options) {
    ...
    if ("includePrivateTlds" in options === false) {
        options.includePrivateTlds = false;
    }
    ...
    if (tld === null) {
        tld = domain.match(options.includePrivateTlds ? knownTlds : knownTlds.icann);
    }
