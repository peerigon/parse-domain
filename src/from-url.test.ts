...
describe(fromUrl.name, () => {
  test("it returns the hostname only", () => {
    expect(fromUrl("https://user@www.example.com:8080/path?query")).toBe("www.example.com");
  });

  test("it handles other protocols well", () => {
    expect(fromUrl("ftp://user@www.example.com:8080/path?query")).toBe("www.example.com");
  });

  test("it accepts incomplete URLs", () => {
    expect(fromUrl("//user@www.example.com:8080/path?query")).toBe("www.example.com");
    expect(fromUrl("user@www.example.com:8080/path?query")).toBe("www.example.com");
  ...
  test("it handles URLs with IPv6", () => {
    expect(fromUrl("http://[1:2:3:4:5:6:7:8]/path?query")).toBe("[1:2:3:4:5:6:7:8]");
  ...
  test("it doesn't get confused with other : characters", () => {
    expect(fromUrl("https://www.example.com/search?updated-max=2020-04-16T09:14:00+10:00")).toBe(
      "www.example.com",
    );
  });
